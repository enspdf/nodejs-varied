...
class BetAmountCommand implements ICommand {
  id: string;
  date: Date;
  amountBet: number;
}
...
class BetAmountCommandHandler
  implements ICommandHandler<BetAmountCommand, BetPlacedEvent>
{
...
class BetPlacedProjector implements IProjector<BetPlacedEvent, UserMoneyState> {
  async project(
    currentState: UserMoneyState,
    event: BetPlacedEvent
  ): Promise<UserMoneyState> {
...
class BetService extends EventBasedService<
  BetAmountCommand,
  BetPlacedEvent,
  UserMoneyState
> {
  constructor() {
    super(
      new BetAmountCommandHandler(),
      new BetPlacedProjector(),
      new LocalEventStore()
    );
  }

  protected async updateState(_state: UserMoneyState): Promise<void> {}
